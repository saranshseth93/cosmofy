...
import { Zap, AlertTriangle, Calendar, Ruler, ExternalLink } from 'lucide-react';
...
export function AsteroidTracker({ id = "asteroids" }: AsteroidTrackerProps) {
  ...
  if (error) {
    return (
      <section id={id} className="py-20 bg-gradient-to-b from-[hsl(244,62%,26%)] to-[hsl(222,47%,8%)]">
      ...
      <div className="container mx-auto px-6">
        <div className="text-center mb-16">
          ...
          <p className="text-xl opacity-80 max-w-2xl mx-auto">
            Monitor potentially hazardous asteroids and upcoming close approaches to Earth with real-time NASA data.
          </p>
        </div>
            ...
            <Card className="glass-effect rounded-xl">
              <CardContent className="p-6 text-center">
                ...
                {isLoading ? (
                  ...
                  <div className="text-2xl font-bold">{dashboardStats.hazardous}</div>
                )}
            ...
            <Card className="glass-effect rounded-xl">
              <CardContent className="p-6 text-center">
                ...
                {isLoading ? (
                  ...
                  <div className="text-2xl font-bold">{dashboardStats.thisWeek}</div>
                )}
            ...
            <Card className="glass-effect rounded-xl">
              <CardContent className="p-6 text-center">
                ...
                {isLoading ? (
                  ...
                  <div className="text-2xl font-bold">{formatDistance(dashboardStats.closest)}</div>
                )}
                  ...
                  <div key={asteroid.id} className="flex flex-col md:flex-row md:items-center justify-between p-6 bg-[hsl(222,47%,8%)] bg-opacity-50 rounded-xl">
                    <div className="flex-1">
                      <div className="flex items-center mb-2 flex-wrap gap-2">
                        ...
                        <Badge 
                          className={asteroid.isPotentiallyHazardous ? 
                            "bg-[hsl(43,96%,56%)]" : 
                            "bg-[hsl(158,76%,36%)]"
                          }
                        >
                          {asteroid.isPotentiallyHazardous ? 'Potentially Hazardous' : 'Safe Passage'}
                        </Badge>
                      </div>
                      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 text-sm">
                        <div>
                          <div className="opacity-70">Date</div>
                          <div className="font-semibold">{formatDate(asteroid.closeApproachDate)}</div>
                        </div>
                        <div>
                        ...
                        <div>
                          ...
                          <div className="font-semibold">{formatSize(asteroid.estimatedDiameter)}</div>
                        </div>
                        <div>
