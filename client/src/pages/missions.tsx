...
export default function SpaceMissions() {
  ...
  const [selectedMission, setSelectedMission] = useState<SpaceMission | null>(
    null
  );
  ...
  const uniqueStatuses = missions
    ? ["All", ...Array.from(new Set(missions.map((m) => m.status)))]
    : ["All"];
      ...
      <section className="pt-24 sm:pt-32 pb-12 sm:pb-16 relative overflow-hidden">
        ...
        <div className="container mx-auto px-4 sm:px-6 relative z-10">
          <div className="max-w-4xl mx-auto text-center">
            <Badge className="mb-4 sm:mb-6 bg-gradient-to-r from-blue-600/20 to-purple-600/20 border border-blue-500/30 text-blue-100 text-xs sm:text-sm">
            ...
            <p className="text-base sm:text-lg lg:text-xl text-gray-300 max-w-2xl mx-auto mb-6 sm:mb-8 px-4 sm:px-0">
              Explore humanity's most ambitious space exploration missions, from
              lunar expeditions to deep space discoveries.
            </p>
          </div>
        </div>
      </section>
          ...
          {isLoading ? (
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
              {filteredMissions.map((mission) => (
                <Card
                  key={mission.id}
                  className="glass-morphism hover:scale-105 transition-all duration-300 cursor-pointer"
                      ...
                      <div className="space-y-2 text-sm">
                        ...
                        <div className="flex items-center text-gray-400">
                          ...
                          <span>
                            ...
                            {getDaysFromNow(mission.launchDate)}
                          </span>
                        </div>
                        {mission.crew && mission.crew.length > 0 && (
                          <div className="flex items-center text-gray-400">
                            ...
                            <span>
                              {mission.crew.length} crew member
                              {mission.crew.length > 1 ? "s" : ""}
                            </span>
                          </div>
                        )}
                      </div>
            ...
            <div className="p-6">
                  ...
                  {selectedMission.instruments && (
                    <div>
                      ...
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-2">
                        {selectedMission.instruments.map(
                          (instrument, index) => (
                            <Badge
                              key={index}
                              variant="outline"
                              className="text-gray-300 border-gray-600 justify-start"
                            >
                              {instrument}
                            </Badge>
                          )
                ...
                <TabsContent value="achievements" className="space-y-4">
                  {selectedMission.keyAchievements && (
                    <div>
                      ...
                      <ul className="space-y-2">
                        {selectedMission.keyAchievements.map(
                          (achievement, index) => (
                            <li key={index} className="flex items-start">
                  ...
                  {selectedMission.keyDiscoveries && (
                    <div>
                      ...
                      <ul className="space-y-2">
                        {selectedMission.keyDiscoveries.map(
                          (discovery, index) => (
                            <li key={index} className="flex items-start">
              ...
              {selectedMission.websiteUrl && (
                <div className="mt-6 pt-6 border-t border-gray-700">
                  <Button
                    onClick={() =>
                      window.open(selectedMission.websiteUrl, "_blank")
                    }
